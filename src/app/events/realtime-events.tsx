...
export default function RealtimeEvents() {
    ...
    const loadEvents = async () => {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/events`, {params: {page: eventPageInfo.page + 1}})
    ...
    return (
        <div>
            {events.map((event)=>(
                <div key={event.id} className="flex mb-4 items-center border-2 rounded p-2">
                    <div>
                        ...
                        {event.hosts?.map((host)=>(
                            <div key={`${event.id}-${host.id}`} className="flex items-center">
                                <Image className="m-3" width={50} height={50} src={host.avatar} alt={host.name}/>
                                <p>{host.name}</p>
                            </div>
                        ))}
                    </div>
            ...
            <button className="flex-no-shrink p-2 border-2 rounded" onClick={loadEvents}>Load More</button>
        </div>);
}